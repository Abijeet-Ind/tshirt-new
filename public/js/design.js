...
document.querySelector('#sellThis').addEventListener('click', el => {
    ...
    html2canvas(divToCapture).then(function (canvas) {
        html2canvas(document.getElementById("product")).then(function (canvas) {
            ...
            sessionStorage.setItem(`designed${$('.btn-toggle')[0].innerText}View`, img); // save the tshirt front view in the session storage
            // alert(`designed${$('.btn-toggle')[0].innerText}View`);
            setTimeout(() => { // saves back tshirt view in 2500ms (2.5s) delay
                // url("product_img/tshirt_back.png")
                $('.front').css("display", "none");
                $('.back').css("display", "block");
